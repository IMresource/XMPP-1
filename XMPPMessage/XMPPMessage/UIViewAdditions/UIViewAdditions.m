...
+ (UIButton *)backButtonWithTarget:(id)target
    ...
    btn.frame = CGRectMake(0, 0, 16, 24);
...
//按钮
+ (UIButton *)pushbuttonWithFrame:(CGRect)frame
                            title:(NSString *)title
                           target:(id)target
                           action:(SEL)action{
    UIButton *btn = [UIButton buttonWithType:UIButtonTypeCustom];
    btn.frame = frame;
//    btn.backgroundColor = K_COLOR_MAINATTUNE;
                             ...
                             action:(SEL)aAction
{
    UIButton * btn = [UIButton buttonWithType:UIButtonTypeSystem];
    btn.frame = aFrame;
    [btn setImage:[[UIImage imageNamed:aImageName] imageWithRenderingMode:UIImageRenderingModeAlwaysOriginal] forState:UIControlStateNormal];
