...
//    [[XMPPManager shareXMPPManager].stream addDelegate:self delegateQueue:dispatch_get_main_queue()];
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(showMessage) name:K_showMessage object:nil];
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(messageFail) name:K_messageFail object:nil];
...
//    [self.allMessageArray removeAllObjects];
    self.allMessageArray = [NSMutableArray arrayWithCapacity:1];
    // 将结果数组赋值给消息数组
